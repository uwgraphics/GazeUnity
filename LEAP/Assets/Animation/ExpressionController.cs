...
public class ExpressionController : AnimController
{
	...
	public virtual void InstaChangeExpression(string exprName)
	{
		expression = exprName;
		curExpr = expression;
		curExprMTIndex = _morphController.GetMorphChannelIndex(curExpr);
		curMag = magnitude;
	}
	...
	public virtual void UnfixExpression(string exprName)
	{
		...
		{
			curExpr = exprName;
			curExprMTIndex = _morphController.GetMorphChannelIndex(exprName);
			if (fixedExpr.ContainsKey(curExprMTIndex))
			{
	...
	protected override void _Init()
	{
		...
		exprMTIndex = _morphController.GetMorphChannelIndex(expression);
		if (exprMTIndex < 0)
		{
			enabled = false;
			return;
	...
	protected virtual void Update_Base()
	{
		if (curExprMTIndex >= 0)
			...
			_morphController.morphChannels[curExprMTIndex].weight += curMag;
	...
	protected virtual void Update_Maximum()
	{
		...
		else if (curTime >= curOnsetTime && curTime < curOutsetTime)
		{
			...
			_morphController.morphChannels[curExprMTIndex].weight += maxMagnitude;
		}
	...
	protected virtual void Update_Changing()
	{
		...
		float t2 = t * t;
		if (curExprMTIndex >= 0)
			_morphController.morphChannels[curExprMTIndex].weight += curMag * (1f + 2 * t2 * t - 3 * t2);
		_morphController.morphChannels[exprMTIndex].weight += mag * (-2 * t2 * t + 3 * t2);
