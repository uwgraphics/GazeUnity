...
public class GazeController : AnimController
{
	...
	public virtual Vector3 EffGazeTargetPosition
	{
		get
		{
			return curGazeTarget.transform.position;
		}
	}
	...
	public virtual void _CalculateMaxVelocities()
	{
		if (Torso != null && curUseTorso)
		{
			...
			float torsoDistRotAlign = Torso.distRotAlign;
		...
		// For the head joints
		float headDistRotAlign = Head.distRotAlign;
	...
	public virtual float _ComputeGazeShiftAmplitude()
	{
		...
		Vector3 trgPos = EffGazeTargetPosition;
	...
	protected virtual void LateUpdate_NoGaze()
	{
		...
		//HACK ALERT:
		if (faceCtrl.randomMotionEnabled)
			faceCtrl.randomMotionGen.LateApply();
		if (faceCtrl.speechMotionEnabled)
			faceCtrl.speechMotionGen.LateApply();
